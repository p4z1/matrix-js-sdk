...
describe("MatrixClient", function() {
    ...
    describe("default client", () => {
        ...
        describe("support for ignoring invites", () => {
            ...
            it(
                "should reject invites once we have added a matching rule in the target room (scope: user)",
                async () => {
            ...
            it(
                "should reject invites once we have added a matching rule in the target room (scope: server)",
                async () => {
            ...
            it(
                "should reject invites once we have added a matching rule in the target room (scope: room)",
                async () => {
                    const REASON = `Just a test ${Math.random()}`;
        ...
        it("supportsRelationBasedRedactions() should return false", async () => {
            expect(await client.supportsRelationBasedRedactions()).toBe(false);
        });
    });

    describe("when the server supports relation based redactions (unstable)", () => {
        beforeEach(() => {
            unstableFeatures[UNSTABLE_MSC3912_RELATION_BASED_REDACTIONS.unstable!] = true;
            makeClient();
        });

        it("supportsRelationBasedRedactions() should return true", async () => {
            expect(await client.supportsRelationBasedRedactions()).toBe(true);
        });

        describe("redactEvent", () => {
            const roomId = "!room:example.org";

            it("should redact with relations if using the with_relations option", async () => {
                const eventId = "$eventId:example.org";
                const txnId = client.makeTxnId();
    ...
    describe("when the server supports relation based redactions (stable)", () => {
        beforeEach(() => {
            unstableFeatures[UNSTABLE_MSC3912_RELATION_BASED_REDACTIONS.stable!] = true;
            makeClient();
        });
